		...
		if (d.containsKey("subject_id")) {
			if (!d.get("subject_id").equals("")) {
				if (((String) d.get("subject_id")).length() <= 8) {
					...
					// validate parameter 'quiz_obj_id'
					if (d.containsKey("quiz_obj_id")) {
						if (!d.get("quiz_obj_id").equals("")) {
							boolean parseUnsignedIntError;
							
							// try to parse 'quiz_obj_id' into unsigned integer
							try {
								Integer.parseUnsignedInt((String) d.get("quiz_obj_id"));
							...
							if (!parseUnsignedIntError) {
								if (Integer.parseUnsignedInt((String) d.get("quiz_obj_id")) <= 2147483647) {
								...
								rc.put("error_code", 400);
								rc.put("description", "Bad Request: 'quiz_obj_id' must be an unsigned integer");
							}
						} else {
							rc.put("error_code", 400);
							rc.put("description", "Bad Request: 'quiz_obj_id' can't be empty");
						}
					} else {
						rc.put("error_code", 400);
						rc.put("description", "Bad Request: Parameter 'quiz_obj_id' is required");
					}
				} else {
					rc.put("error_code", 400);
...
if (execute) {
	...
	if (workload != null) {
		QuizObjective quizObjective = lecturerUser.getQuizObj(Integer.parseUnsignedInt((String) d.get("quiz_obj_id")), workload.getId());
		
		if (quizObjective != null) {
			boolean ok = lecturerUser.updateQuizObj(quizObjective.getId(), (String) d.get("question"), (String) d.get("choice_a"), (String) d.get("choice_b"),
			...
			if (ok) {
				lecturerUser.addLogRecord("UPDATE", "[" + sdf.format(new Date()) + "] Lecturer " + (String) session.getAttribute("user_id") +
						" updated quiz objective question");
			...
			rc.put("error_code", 400);
			rc.put("description", "Bad Request: The corresponding quiz objective doesn't exist");
		}
	} else {
		rc.put("error_code", 400);
