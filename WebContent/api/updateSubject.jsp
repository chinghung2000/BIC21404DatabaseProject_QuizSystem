...
if (validate) {
	...
	if (session.getAttribute("user_id") != null && session.getAttribute("user_type").equals("admin")) {
		...
		// validate parameter 'old_subject_id'
		if (d.containsKey("old_subject_id")) {
			if (!d.get("old_subject_id").equals("")) {
				if (((String) d.get("old_subject_id")).length() <= 8) {
					...
					rc.put("error_code", 400);
					rc.put("description", "Bad Request: 'old_subject_id' length can't be more than 8");
				}
			} else {
				rc.put("error_code", 400);
				rc.put("description", "Bad Request: 'old_subject_id' can't be empty");
			}
		} else {
			rc.put("error_code", 400);
			rc.put("description", "Bad Request: Parameter 'old_subject_id' is required");
		}
	} else {
